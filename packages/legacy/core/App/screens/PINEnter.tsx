...
import { EventTypes, attemptLockoutBaseRules, attemptLockoutThresholdRules } from "../constants";
import { DispatchAction } from "../contexts/reducers/store";
import { useTheme } from "../contexts/theme";
import { useAnimatedComponents } from "../contexts/animated-components";
import { BifoldError } from "../types/error";
...
//   // 'You're logged out' popup modal
          ...
          const clearAlertModal = () => {
            switch (usage) {
              ...
              default:
                setAlertModalVisible(false);
                setLoading(false)
                // Focus on the TextInput after a short delay
                setTimeout(() => {
                  textInputRef.current?.focus();
                }, 100);
                break;
  ...
  return (
    <SafeAreaView>
     ...
     <Text style={{ fontSize: getFontSizel(), alignSelf: 'center', color: '#8E8E8E', marginTop: '2%', marginBottom: '10%' }}>Enter PIN to unlock the wallet</Text>
            ...
            <TextInput
               ...
               keyboardType="numeric"
                maxLength={6}
                      ...
                      onChangeText={handlePinChange}
                      onKeyPress={handleKeyPress}
                      onSubmitEditing={() => pin.length === 6 && (onPINInputCompleted(pin))}
                      autoFocus
                      blurOnSubmit={showError}
                  />
              ...
              {Loading && <ActivityIndicator/>}
                            ...
                            <PopupModal
                              ...
                              bodyContent={
                                <View>
                                  <Text style={styles.modalText}>{t('PINEnter.LoggedOutDescription')}</Text>
                                </View>
                              }
                              onCallToActionLabel={t('Global.Okay')}
                              onCallToActionPressed={() => {
                  ...
                  {alertModalVisible && (
                      <PopupModal
                        ...
                        bodyContent={
                          <View>
                            <Text style={styles.modalText}>{t('PINEnter.RepeatPIN')}</Text>
                            {displayLockoutWarning ? (
                              <Text style={styles.modalText}>{t('PINEnter.AttemptLockoutWarning')}</Text>
                            ) : null}
                          </View>
                        }
                        onCallToActionLabel={t('Global.Okay')}
