...
const Chat: React.FC<ChatProps> = ({ route }) => {
  ...
  return (
    <SafeAreaView edges={['bottom', 'left', 'right']} style={{ flex: 1, paddingTop: 20 }}>
        <View style={{display:'flex',flexDirection:'row',justifyContent:'flex-start',alignItems:'center',width:'90%',marginHorizontal:'8%'}} >
                        <AntDesign name="arrowleft" size={24} color="black" style={{width:'10%'}} onPress={()=>navigation.goBack()}/>
                        <View style={{display:'flex' ,flexDirection:'row',justifyContent:'flex-start',alignItems:'center',width:'90%',gap:5}}>
                            <University/>

                            <Text style={{color:'black',marginBottom:1,fontSize:16}} >{theirLabel}</Text>
                        </View>
